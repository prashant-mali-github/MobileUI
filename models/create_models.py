...
class Customer_Orders(db.Model):
    ...
    o_id = db.Column(db.Integer(), db.ForeignKey('sales_items.id', ondelete='CASCADE'))
    sale_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    customer = db.relationship('Customer', backref=db.backref("customer_orders", cascade="all, delete-orphan"))
    sale_item = db.relationship('SalesItems', backref=db.backref("customer_orders", cascade="all, delete-orphan"))
    def __init__(self, c_id=None, o_id=None):
        self.c_id = c_id
        self.o_id = o_id
